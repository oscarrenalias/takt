...
- **Should operator and worker skills diverge?** Both operator and worker access the same `docs/memory/` DB and files — shared memory, single source of truth. The skill *instructions* differ: the worker skill mandates `takt memory search "<task>"` at bead start and writes on discovery; the operator skill uses on-demand retrieval and has full write access including superseding or correcting entries written by agents. The operator is the most senior role in the hierarchy and should be able to shape institutional memory across all agent runs. **Resolved: same memory, split skill instructions.**
- **Embedding model in schema**: no model tracking needed. `takt memory rebuild` re-embeds everything from scratch, so changing `embedding_model` in config followed by a rebuild is sufficient. Docs should note that changing the model requires a rebuild. **Resolved: no model tracking in schema.**
