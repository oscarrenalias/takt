...
src/agent_takt/
  ...
  scheduler/      Orchestration loop package: leases, conflicts, followups (all params from config)
    ...
    finalize.py   Bead finalization and status transitions; strips `.takt/beads/` paths from worker-reported `touched_files`/`changed_files` via `_strip_bead_state_paths()` before persisting to the bead and handoff summary
...
### Cleaning bead-state history in an existing repo

To stop accumulating `[bead]` commits in an existing repo and optionally remove the ones already there:

1. Set `common.commit_bead_state: false` in `.takt/config.yaml` and commit the change.
2. Append `.takt/beads/` to `.gitignore` and commit.
3. Run `git rm -r --cached .takt/beads/` and commit — this untracks the bead-state directory from the index without deleting the files on disk.
4. *(Optional, destructive)* To purge the historical `[bead]` commits from the log entirely, use `git filter-repo` or an interactive rebase. This rewrites history and requires a force-push; it is out of scope for the `takt` tool itself.
...
- File-scope conflicts are checked statically at schedule time. Overlapping `expected_files`/`expected_globs` between in-progress beads in the same feature tree cause blocking.
...
- **Bead state exclusion**: Bead state isolation uses two layers. (1) `_write_worktree_exclude` writes `.takt/beads/` to `.git/worktrees/<name>/info/exclude` when a feature worktree is created, hiding untracked bead files from `git status` for operator ergonomics. (2) `WorktreeManager.commit_all()` enumerates changed paths via `git status --porcelain=v1` and filters out any `.takt/beads/` entries before staging, so worker commits never include bead state files regardless of whether `.takt/beads/` is gitignored. Bead state files remain tracked in the feature branch index (inherited from main); isolation is achieved only at the staging layer. The `.gitattributes` rule `.takt/beads/** merge=ours` provides an additional safety net, and `WorktreeManager.merge_main_into_branch()` plus `merge_branch()` now enforce that policy explicitly by auto-resolving any `.takt/beads/` merge conflicts in favor of the current branch before completing the merge. In practice that means preflight merges keep the feature branch's bead snapshot, while final merges on `main` keep `main`'s bead snapshot; the fallback only commits automatically when every remaining conflict is under `.takt/beads/`. `merge_main_into_branch` additionally guards against the "would be overwritten by merge" error: before merging, it saves and removes any untracked `.takt/beads/` files (those not in the git index) via `_save_and_remove_bead_files`, then restores them unconditionally in a `try/finally` block via `_restore_saved_bead_files` — so the feature worktree's local bead snapshot is preserved regardless of whether the merge succeeds or fails. Tracked bead files are left alone and flow through git's normal merge path. If no paths remain after filtering (e.g. when only `.takt/beads/` files changed), `commit_all` returns `None` without attempting a commit. **Finalize-time path stripping**: `BeadFinalizer.finalize()` and `_handle_recovery_completion()` both call `_strip_bead_state_paths()` as a defensive safety net, removing any path starting with `.takt/beads/` before the lists are written to the bead JSON and handoff summary. This prevents bead-state noise from inflating every bead record and propagating through followup scope population. When `common.commit_bead_state` is `false`, `_git_commit_bead` and `_git_commit_bead_deletion` are no-ops; the worktree-exclude and `merge=ours` mechanisms remain active and are still useful for repos that keep the flag on.
