...
def generate_config_yaml(answers: InitAnswers) -> str:
    ...
    """
    return (
        "# takt configuration — generated by `takt init`.\n"
        ...
        f"  # max_workers is a CLI flag: takt run --max-workers {answers.max_workers}\n"
        "\n"
        "codex:\n"
        "  binary: codex\n"
        "  skills_dir: .agents\n"
...
def scaffold_project(
    ...
    *,
    ...
    if update_gitignore(project_root):
        _log("Updated .gitignore with takt entries")
    else:
    ...
    out.write("\nDone. Run `takt summary` to verify the setup.\n")
    out.flush()
