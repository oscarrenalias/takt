...
class CommitAllPathspecExclusionTests(OrchestratorTests):
    """Verify WorktreeManager.commit_all excludes .takt/beads/ via pathspec exclusion."""

    def test_commit_all_excludes_bead_file_from_commit_diff(self) -> None:
        """commit_all must not include a tracked+modified .takt/beads/ file in the resulting commit."""
        worktree_path = self.root

        # Stage and commit a bead file so it is tracked by git
        bead_dir = worktree_path / ".takt" / "beads"
        bead_dir.mkdir(parents=True, exist_ok=True)
        bead_file = bead_dir / "B-foo.json"
        bead_file.write_text('{"bead_id": "B-foo"}', encoding="utf-8")
        subprocess.run(
            ["git", "add", str(bead_file)],
            cwd=worktree_path,
            check=True,
            capture_output=True,
        )
        subprocess.run(
            ["git", "commit", "-m", "track bead state"],
            cwd=worktree_path,
            check=True,
            capture_output=True,
        )

        # Modify the tracked bead file on disk (simulates in-flight scheduler writes)
        bead_file.write_text('{"bead_id": "B-foo", "status": "in_progress"}', encoding="utf-8")

        # Create a real source file that commit_all should include
        real_file = worktree_path / "worker_output.py"
        real_file.write_text("# generated by worker\n", encoding="utf-8")

        wm = WorktreeManager(worktree_path, worktree_path / ".takt" / "worktrees")
        sha = wm.commit_all(worktree_path, "chore: worker changes")

        self.assertIsNotNone(sha, "commit_all should produce a commit when real files are staged")

        # Inspect the set of files changed in the new commit
        ...
        committed_files = diff_result.stdout.splitlines()

        self.assertIn("worker_output.py", committed_files)
        self.assertNotIn(
            ".takt/beads/B-foo.json",
            committed_files,
            ".takt/beads/B-foo.json must be excluded from commit_all via pathspec",
        )
